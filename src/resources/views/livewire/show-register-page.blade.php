                ...
                <form wire:submit.prevent="payAndRegister">
                    @foreach(['name','student_id','student_origin','email','phone','address','github_name'] as $field)
                        <div class="form-group mb-3">
                            ...
                            >
                            @error($field) <span class="text-danger text-sm">{{ $message }}</span> @enderror
                        </div>
                    @endforeach
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input
                            ...
                            readonly
                        >
                        @error('github_url') <span class="text-danger text-sm">{{ $message }}</span> @enderror
                    </div>

                    <button type="submit" class="btn btn-primary w-100">Pay & Register</button>
                </form>
            </div>
        </div>
    </div>
